...
async def tratar_resposta_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if etapa == 'tema':
        ...
        await enviar_opcoes_perguntas(update.message)
...
async def tratar_callback_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("qtd_"):
        ...
        await confirmar_quiz(query.message, context)
        ...
        keyboard = [[InlineKeyboardButton("▶️ Iniciar Quiz", callback_data="iniciar_quiz")]]
        await query.message.reply_text("✅ Quiz gerado com sucesso! Pronto para iniciar.", reply_markup=InlineKeyboardMarkup(keyboard))

# 📊 Botões: número de perguntas
async def enviar_opcoes_perguntas(message):
    keyboard = [
    ...
    await message.reply_text("📊 Quantas perguntas?", reply_markup=InlineKeyboardMarkup(keyboard))

# 🔢 Botões: número de alternativas
async def enviar_opcoes_alternativas(message):
    keyboard = [
    ...
    await message.reply_text("🔢 Quantas alternativas por pergunta?", reply_markup=InlineKeyboardMarkup(keyboard))

# ⏱️ Botões: tempo por pergunta
async def enviar_opcoes_tempo(message):
    keyboard = [
    ...
    await message.reply_text("⏱️ Tempo por pergunta?", reply_markup=InlineKeyboardMarkup(keyboard))

# 🎯 Botões: dificuldade
async def enviar_opcoes_dificuldade(message):
    keyboard = [
    ...
    await message.reply_text("🎯 Escolha a dificuldade:", reply_markup=InlineKeyboardMarkup(keyboard))

# ✅ Confirmação final
async def confirmar_quiz(message, context: ContextTypes.DEFAULT_TYPE):
    dados = context.user_data
    resumo = (
        f"✅ Tema: {dados['tema']}\n"
    ...
    await message.reply_text(resumo, reply_markup=InlineKeyboardMarkup(keyboard))
